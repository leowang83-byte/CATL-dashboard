...
def main():
        ...
        if price_forecast_df.empty:
            ...
            with chart_col:
                ...
                if price_timeseries_df.empty:
                    ...
                    if not actual_plot_df.empty and not forecast_plot_df.empty:
                        ...
                        if last_actual["date"] < first_forecast_date:
                            bridge_row = {
                                ...
                                "forecast_upper": min(PAPER_AISC_90_HIGH_WAN, last_actual["actual_lce_price"] + 0.30),
                            }
                    ...
                    if not forecast_plot_df.empty and "forecast_upper" in forecast_plot_df.columns:
                        fig_ts.add_trace(
                            go.Scatter(
                                x=forecast_plot_df["date"],
                                y=forecast_plot_df["forecast_upper"],
                                mode="lines",
                                name="预测上沿",
                                line=dict(
                                    color=TEAL_LIGHT,
                    ...
                    if not forecast_plot_df.empty and "forecast_lower" in forecast_plot_df.columns:
                        fig_ts.add_trace(
                            go.Scatter(
                                x=forecast_plot_df["date"],
                                y=forecast_plot_df["forecast_lower"],
                                mode="lines",
                                name="预测下沿",
                                line=dict(
                                    color="#FF8A65",
    ...
    if top_nav == "CATL资源事业部周报":
        ...
        render_section_02_price_forecast(current_price_wan)
