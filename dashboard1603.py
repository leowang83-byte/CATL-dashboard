...
def render_catl_procurement_engine(forecast, price_zone=""):
    """Render a cleaner CATL buyer-side procurement panel for the LCE price page."""
    ...
    if reference_price_wan <= PAPER_PRICE_LOW_WAN:
        price_regime = "补库窗口"
        base_action = "低位区间，适合提前补库并争取长协锁量。"
        intensity = 85
        accent = "#16A34A"
        accent_bg = "#ECFDF5"
    elif reference_price_wan <= PAPER_PRICE_CENTER_WAN:
        price_regime = "分批采购窗口"
        base_action = "价格仍具性价比，宜分批建仓，保留回调补库空间。"
        intensity = 68
        accent = CATL_BLUE
        accent_bg = "#EFF6FF"
    elif reference_price_wan <= PAPER_PRICE_HIGH_WAN:
        price_regime = "中性震荡区间"
        base_action = "以按需采购为主，同时观察期现结构寻找锁价点。"
        intensity = 48
        accent = "#D97706"
        accent_bg = "#FFFBEB"
    else:
        price_regime = "高位风险区"
        base_action = "减少追高采购，优先控制敞口，等待更好回落窗口。"
        intensity = 25
        accent = "#C2410C"
        accent_bg = "#FFF7ED"
    ...
    if inventory_days > 0:
        if inventory_days <= PAPER_INVENTORY_THRESHOLD_DAYS:
            ...
            inventory_action = "库存中性偏低，可执行分批补库。"
            intensity += 4
        elif inventory_days <= 30:
            inventory_action = "库存仍可覆盖短期需求，维持按需采购。"
        else:
            inventory_action = "库存偏高，应控制现货采购节奏。"
            intensity -= 12
    else:
        inventory_action = "库存数据未接入，等待库存信号确认。"
        intensity -= 5

    if basis_wan is None:
        basis_text = "待接入"
        hedge_action = "先补齐GFEX与现货价差，再判断套保方向。"
    elif basis_wan >= 0.30:
        basis_text = f"升水 {basis_wan:+.2f} 万元/吨"
        hedge_action = "期货升水，市场预期偏强，可评估提前锁价。"
        intensity += 5
    elif basis_wan <= -0.30:
        basis_text = f"贴水 {basis_wan:+.2f} 万元/吨"
        hedge_action = "期货贴水，市场预期偏弱，除安全库存外不宜追高。"
        intensity -= 5
    else:
        basis_text = f"平衡 {basis_wan:+.2f} 万元/吨"
        hedge_action = "期现结构中性，维持滚动采购。"

    intensity = max(0, min(intensity, 100))
    if intensity >= 75:
        final_action = "主动补库"
        action_subtitle = "低位窗口明确，可提高锁量比例。"
    elif intensity >= 55:
        final_action = "分批采购"
        action_subtitle = "保留节奏感，兼顾安全库存与价格弹性。"
    elif intensity >= 35:
        final_action = "按需采购"
        action_subtitle = "以刚需覆盖为主，不扩大额外敞口。"
    else:
        final_action = "控制敞口 / 等待回落"
        action_subtitle = "避免高位集中采购，优先风险管理。"
    ...
    inventory_display = f"{inventory_days:.1f}天" if inventory_days > 0 else "N/A"
    ...
    action_html = "".join(
        ...
        f'<div style="width:18px;height:18px;border-radius:999px;background:{accent_bg};color:{accent};'
        f'display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:900;flex:0 0 18px;">•</div>'
        f'<div style="font-size:13px;line-height:1.55;color:{TEXT_DARK};">{html.escape(item)}</div>'
        f'</div>'
    ...
    st.markdown(
        f"""
        <div style="background:#FFFFFF;border:1px solid {BORDER};border-left:6px solid {accent};border-radius:14px;padding:16px 16px 14px 16px;box-shadow:0 2px 8px rgba(0, 58, 140, 0.06);margin-bottom:12px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap;margin-bottom:8px;">
                <div>
                    <div style="font-size:12px;color:{TEXT_MUTED};font-weight:800;letter-spacing:0.2px;">当前动作</div>
                    <div style="font-size:18px;color:{accent};font-weight:900;line-height:1.25;margin-top:3px;">{html.escape(final_action)}</div>
                    <div style="font-size:13px;color:{TEXT_DARK};line-height:1.5;margin-top:4px;">{html.escape(action_subtitle)}</div>
                </div>
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    <span style="display:inline-block;padding:5px 10px;border-radius:999px;background:{accent_bg};color:{accent};font-size:12px;font-weight:900;">{html.escape(price_regime)}</span>
                    <span style="display:inline-block;padding:5px 10px;border-radius:999px;background:#F8FAFC;color:{TEXT_MUTED};font-size:12px;font-weight:800;">{html.escape(price_zone or '价格区间判断')}</span>
                </div>
            </div>
            {action_html}
        </div>
    ...
    with row1_col1:
        st.markdown(
            f"""
            <div style="background:#FFFFFF;border:1px solid {BORDER};border-radius:12px;padding:12px 14px;min-height:102px;box-shadow:0 2px 6px rgba(0, 58, 140, 0.05);">
                <div style="font-size:13px;color:{TEXT_MUTED};font-weight:800;">采购强度</div>
                <div style="font-size:22px;color:{CATL_BLUE};font-weight:900;margin-top:6px;">{intensity:.0f}/100</div>
                <div style="font-size:12px;color:{TEXT_MUTED};margin-top:5px;">0=观望，100=主动补库</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with row1_col2:
        st.markdown(
            f"""
            <div style="background:#FFFFFF;border:1px solid {BORDER};border-radius:12px;padding:12px 14px;min-height:102px;box-shadow:0 2px 6px rgba(0, 58, 140, 0.05);">
                <div style="font-size:13px;color:{TEXT_MUTED};font-weight:800;">价格区间</div>
                <div style="font-size:22px;color:{CATL_BLUE};font-weight:900;margin-top:6px;">{html.escape(price_regime)}</div>
                <div style="font-size:12px;color:{TEXT_MUTED};margin-top:5px;">参考价 {reference_price_wan:.2f} 万元/吨</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        f"""
        <div style="background:#F8FAFC;border:1px solid {BORDER};border-radius:12px;padding:14px 15px;margin-top:10px;margin-bottom:12px;">
            <div style="font-size:13px;color:{TEXT_MUTED};font-weight:800;margin-bottom:8px;">触发信号</div>
            <div style="font-size:13px;color:{TEXT_DARK};line-height:1.75;">
                <b>预测区间：</b>{html.escape(price_range_text)}<br>
                <b>库存状态：</b><span style="color:{inventory_color};font-weight:900;">{html.escape(inventory_status)}</span>（{inventory_display}）<br>
                <b>期现结构：</b>{html.escape(basis_text)}<br>
                <b>SC6：</b>{html.escape(sc6_text)} ｜ <b>正极开工率：</b>{html.escape(utilization_text)}
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def main():
    ...
    def render_section_02_price_forecast():
        ...
        if price_forecast_df.empty:
            ...
            with chart_col:
                ...
                if price_timeseries_df.empty:
                    ...
                    if y_values:
                        ...
                        # 从14万元/吨开始展示纵轴，避免0轴拉低图表信息密度。
                        y_range = [
                            14,
                            max(y_max + y_padding, 22),
                        ]
                    else:
                        y_range = [14, 22]

                    y_tick_upper = int(max(y_range[1], 22))
                    y_tick_values = list(range(14, y_tick_upper + 1, 2))
            ...
            st.markdown("#### 核心输入")
            input_m1, input_m2, input_m3 = st.columns(3)
