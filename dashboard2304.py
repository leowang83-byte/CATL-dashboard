...
def subsection_title(title, subtitle=""):
    st.markdown(
        ...
        ">
            <div style="font-size:20px;font-weight:900;color:#003A8C;">{title}</div>
...
def main():
    ...
    def render_section_00_weekly_report():
        ...
        if current_inventory_days > 0:
            inventory_status_tag = signal_tag(inventory_signal["status"])
            inventory_summary_parts = [
                f"当前碳酸锂库存天数约 {current_inventory_days:.1f} 天，处于{inventory_signal['status']}状态。"
            ]
    ...
    def render_section_02_price_forecast():
        ...
        if price_forecast_df.empty:
            ...
            st.markdown(
                ...
                库存天数用于捕捉短周期去库/累库信号，90% AISC 构成边际成本支撑带，
                GFEX 期货价格用于反映市场预期。
                </div>
                """,
                unsafe_allow_html=True,
            ...
            with chart_col:
                ...
                if price_timeseries_df.empty:
                    ...
                    # 对未来预测段进行系统锚定校准：
                    # 预测中枢不应长期突破17.0–19.5万元/吨的投资决策区间
                    if "forecast_center" in ts_df.columns:
                        ts_df["forecast_center"] = ts_df["forecast_center"].clip(
                            lower=PAPER_PRICE_LOW_WAN,
                            upper=PAPER_PRICE_HIGH_WAN,
                        )
    
                    if "forecast_lower" in ts_df.columns:
                        ts_df["forecast_lower"] = ts_df["forecast_lower"].clip(
                            lower=PAPER_PRICE_LOW_WAN,
                            upper=PAPER_PRICE_HIGH_WAN,
                        )
    
                    if "forecast_upper" in ts_df.columns:
                        ts_df["forecast_upper"] = ts_df["forecast_upper"].clip(
                            lower=PAPER_PRICE_LOW_WAN,
                            upper=PAPER_AISC_90_HIGH_WAN,
                        )
                    ...
                    forecast_band_df = forecast_plot_df.copy()
                    ...
                    # 预测区间阴影只从正式预测月开始，避免历史衔接处形成突兀斜边。
                    if not forecast_band_df.empty and "forecast_upper" in forecast_band_df.columns:
                        fig_ts.add_trace(
                            go.Scatter(
                                x=forecast_band_df["date"],
                                y=forecast_band_df["forecast_upper"],
                                mode="lines",
                                name="预测上沿",
                                line=dict(
                                    color="rgba(0, 171, 150, 0.24)",
                                    width=1,
                                ),
                                hovertemplate=
                                    "日期：%{x|%Y-%m-%d}<br>"
                    ...
                    if not forecast_band_df.empty and "forecast_lower" in forecast_band_df.columns:
                        fig_ts.add_trace(
                            go.Scatter(
                                x=forecast_band_df["date"],
                                y=forecast_band_df["forecast_lower"],
                                mode="lines",
                                name="预测下沿",
                                line=dict(
            ...
            with strategy_col:
                ...
                st.markdown(
                    ...
                    <span style="color:{inventory_signal['color']};font-weight:800;">
                    库存状态：{inventory_signal['status']}
                    </span>
                    </div>
                    """,
                    unsafe_allow_html=True,
                ...
                if current_price_wan >= PAPER_PRICE_HIGH_WAN:
                    procurement_strategy = (
                        "价格处于区间上沿附近，采购侧不宜追高边际资源。建议优先锁定低成本长协与权益资源，"
                        "同步评估高成本供给复产带来的回落风险。"
                    )
                elif current_price_wan >= aisc_90_wan:
                    procurement_strategy = (
                        "价格仍覆盖AISC90成本锚，但安全垫需要持续验证。建议保持谨慎采购节奏，"
                        "优先谈判低成本资源、长协条款与库存周转弹性。"
                    )
                else:
                    procurement_strategy = (
                        "价格低于AISC90成本锚，需关注高成本供给退出和项目延期风险。"
                        "对宁德时代而言，可跟踪战略锁量窗口，但应避免承担短期价格下行风险。"
                    )

                st.markdown("#### 采购与资源配置建议")
                st.markdown(
                    f"""
                    <div class="strategy-box">
                    {procurement_strategy}
    ...
    def render_section_03_supply_demand():
        section_header("03", "2026–2035中期预测")
        section_open()
    ...
    def render_section_05_aisc_curve():
        ...
        if not unit_audit_df.empty:
            with st.expander("AISC单位校验（元/吨 → 万元/吨）", expanded=False):
                ...
                st.dataframe(unit_audit_df.drop_duplicates(subset=["字段"], keep="last"), use_container_width=True, hide_index=True)
        ...
        st.plotly_chart(fig_aisc, use_container_width=True)
        st.caption(
        ...
        st.dataframe(top_df[["项目", "国家", "原始AISC（万元/吨）", "政策风险溢价（万元/吨）", "战略AISC（万元/吨）", "政策风险溢价", "投资判断", "调整原因"]], use_container_width=True, hide_index=True)
    ...
    def render_section_06_resource_portfolio_allocation():
        ...
        st.dataframe(top_projects_display, use_container_width=True, hide_index=True)
    ...
    def render_section_06_policy_risk():
        ...
        if scored_policy_table is not None and not scored_policy_table.empty:
            ...
            if policy_type_col and stage_col and risk_col:
                ...
                if not matrix_df.empty:
                    ...
                    st.plotly_chart(fig_policy_matrix, use_container_width=True)
                else:
        ...
        if not top_events_df.empty:
            st.dataframe(top_events_df.head(10), use_container_width=True, hide_index=True)
        else:
        ...
        if strategic_aisc_policy_df is not None and not strategic_aisc_policy_df.empty:
            if "policy_risk_premium_wan" in strategic_aisc_policy_df.columns:
                ...
                if not premium_top_df.empty:
                    ...
                    st.dataframe(premium_display_df, use_container_width=True, hide_index=True)
                else:
        ...
        st.dataframe(data_status_df, use_container_width=True, hide_index=True)
    ...
    def render_section_09_data_model():
        ...
        st.dataframe(rpi_tier_df, use_container_width=True, hide_index=True)
        ...
        st.dataframe(investment_tier_df, use_container_width=True, hide_index=True)
